...
# 只添加这一行：导入你的 PolaLinearAttention
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolov8n.yaml", ch=3, nc=None, verbose=True):
        ...
        # ========== 修复：将自定义模块加入全局字典（无需 model_dict 参数） ==========
        from ultralytics.nn.modules.pola_attention import PolaLinearAttention
        from ultralytics.nn.modules.carafe import CARAFE
        globals()["PolaLinearAttention"] = PolaLinearAttention  # 加入全局变量
        globals()["CARAFE"] = CARAFE  # 加入全局变量

        # Define model（后续代码不变，且删除 parse_model 中的 model_dict 参数）
        ch = self.yaml["ch"] = self.yaml.get("ch", ch)
        if nc and nc != self.yaml["nc"]:
            LOGGER.info(f"Overriding model.yaml nc={self.yaml['nc']} with nc={nc}")
            self.yaml["nc"] = nc
        # 关键：删除 model_dict=self.model_dict 参数
        self.model, self.save = parse_model(deepcopy(self.yaml), ch=ch, verbose=verbose)
        self.names = {i: f"{i}" for i in range(self.yaml["nc"])}
        self.inplace = self.yaml.get("inplace", True)
        self.end2end = getattr(self.model[-1], "end2end", False)

        # 后续的 Build strides、Init weights 等代码保持不变...
